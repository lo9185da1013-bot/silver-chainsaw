...
import net.peyton.eagler.mod.ParticleMod;
...
public class ParticleManager {
	...
	private final List<Particle>[][] fxLayers = new List[4][];
	private final List<ParticleEmitter> particleEmitters = Lists.<ParticleEmitter>newArrayList();
	...
	private final List<Particle> queueEntityFX = Lists.<Particle>newArrayList();
	...
	public Particle spawnEffectParticle(int particleId, double xCoord, double yCoord, double zCoord, double xSpeed,
		...
		// Check ParticleMod settings before spawning
		EnumParticleTypes particleType = EnumParticleTypes.getParticleFromId(particleId);
		if (particleType != null) {
			ParticleMod particleMod = (ParticleMod) net.peyton.eagler.mod.ModManager.getInstance().getMod("Particle Mod");
			if (particleMod != null && particleMod.isEnabled()) {
				if (!particleMod.getParticleSettings().shouldRenderParticle(particleType)) {
					return null; // Don't spawn this particle
				}
			}
		}
	...
	public void updateEffects() {
		...
		if (!this.queueEntityFX.isEmpty()) {
			for (int i = 0; i < this.queueEntityFX.size(); i++) {
				Particle particle = this.queueEntityFX.get(i);
				int j = particle.getFXLayer();
				int k = particle.isTransparent() ? 0 : 1;

				if (this.fxLayers[j][k].size() >= 16384) {
					this.fxLayers[j][k].remove(0);
				}

				this.fxLayers[j][k].add(particle);
				this.queueEntityFX.remove(particle);
			}
		}
	}
	...
	private void tickParticleList(List<Particle> p_187240_1_) {
		if (!p_187240_1_.isEmpty()) {
			for (int i = 0; i < p_187240_1_.size(); i++) {
				Particle particle = p_187240_1_.get(i);
				this.tickParticle(particle);

				if (!particle.isAlive()) {
					p_187240_1_.remove(particle);
				}
			}
		}
	}
	...
	public void renderParticles(Entity entityIn, float partialTicks) {
		...
		List<List<Particle>> visibleParticles = null;
	...
	public void renderLitParticles(Entity entityIn, float partialTick) {
		...
		for (int i = 0; i < 2; ++i) {
			List<Particle> queue = this.fxLayers[3][i];
			...
			for (int j = 0; j < 4; ++j) {
				for (int k = 0; k < 4; ++k) {
					for (int l = 0; l < 4; ++l) {
						...
						this.addEffect(
								(new ParticleDigging(this.worldObj, (double) pos.getX() + d0, (double) pos.getY() + d1,
										(double) pos.getZ() + d2, d0 - 0.5D, d1 - 0.5D, d2 - 0.5D, state))
										.setBlockPos(pos));
					}
				}
			}
	...
	public void addBlockHitEffects(BlockPos pos, EnumFacing side) {
		...
		if (iblockstate.getRenderType() != EnumBlockRenderType.INVISIBLE) {
			...
			double d0 = (double) i
					+ this.rand.nextDouble() * (axisalignedbb.maxX - axisalignedbb.minX - 0.20000000298023224D)
					+ 0.10000000149011612D + axisalignedbb.minX;
			double d1 = (double) j
					+ this.rand.nextDouble() * (axisalignedbb.maxY - axisalignedbb.minY - 0.20000000298023224D)
					+ 0.10000000149011612D + axisalignedbb.minY;
			double d2 = (double) k
					+ this.rand.nextDouble() * (axisalignedbb.maxZ - axisalignedbb.minZ - 0.20000000298023224D)
			...
			this.addEffect((new ParticleDigging(this.worldObj, d0, d1, d2, 0.0D, 0.0D, 0.0D, iblockstate))
	...
	public List<Particle>[][] getFxLayers() {
