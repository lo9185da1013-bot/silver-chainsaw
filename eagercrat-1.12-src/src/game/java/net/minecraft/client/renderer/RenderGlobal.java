...
import net.peyton.eagler.mod.ModManager;
...
public class RenderGlobal implements IWorldEventListener, IResourceManagerReloadListener {
	...
	private Particle func_190571_b(int p_190571_1_, boolean p_190571_2_, boolean p_190571_3_, double p_190571_4_,
		...
		if (this.mc != null && entity != null && this.mc.effectRenderer != null) {
			...
			if (p_190571_2_) {
				EnumParticleTypes _ept = EnumParticleTypes.getParticleFromId(p_190571_1_);
				boolean _allowed = true;
				try {
					net.peyton.eagler.mod.BaseMod _bm = ModManager.getInstance().getMod("Particle Mod");
					if (_bm instanceof ParticleMod) {
						ParticleMod _pm = (ParticleMod) _bm;
						if (_ept != null && _pm.isEnabled()) {
							_allowed = _pm.getParticleSettings().shouldRenderParticle(_ept);
						}
					}
				} catch (Throwable t) {
				}
				...
				if (k1 > 1) {
					...
					EnumParticleTypes _ept2 = EnumParticleTypes.getParticleFromId(p_190571_1_);
					boolean _allowed2 = true;
					try {
						net.peyton.eagler.mod.BaseMod _bm2 = ModManager.getInstance().getMod("Particle Mod");
						if (_bm2 instanceof ParticleMod) {
							ParticleMod _pm2 = (ParticleMod) _bm2;
							if (_ept2 != null && _pm2.isEnabled()) {
								_allowed2 = _pm2.getParticleSettings().shouldRenderParticle(_ept2);
							}
						}
					} catch (Throwable t) {
					}
